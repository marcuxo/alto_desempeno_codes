...
int main(int argc, char* argv[]) {
    ...
    int gathered_data[16]; // Buffer para recolectar datos al final
    ...
    if (rank == root) {
        for (int i = 0; i < 16; i++) {
            send_data[i] = i; // Inicializa el arreglo con valores
        }
    ...
    for (int i = 0; i < 4; i++) {
        recv_data[i] *= 2; // Duplicar los valores como ejemplo de procesamiento
    }
    // Recolectar los datos procesados de todos los procesos en el proceso raíz
    MPI_Gather(recv_data, 4, MPI_INT, gathered_data, 4, MPI_INT, root, MPI_COMM_WORLD);
    
    // El proceso raíz imprime los resultados finales
    if (rank == root) {
        printf("Datos recolectados en el proceso raíz: ");
        for (int i = 0; i < 16; i++) {
